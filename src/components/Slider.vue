...
<template>
  <div v-if="imgSize>0" :id="sliderId" :class="{'mui-slider':isImgMoveThenOne}">
    <div class="mui-slider-group mui-slider-loop">
      <!-- 额外增加的一个节点(循环轮播：第一个节点是最后一张轮播) -->
      <div v-if="isImgMoveThenOne" class="mui-slider-item mui-slider-item-duplicate">
        <a :href="lastImg.href">
          <img :src="lastImg.img">
        </a>
      </div>
    ...
    <div v-if="sIndicator === 'num' && isImgMoveThenOne" class="mui-slider-indicator">
      <div class="mui-number">
        <span>1</span> / {{imgSize}}
      </div>
    </div>
    <div v-else-if="isImgMoveThenOne" class="mui-slider-indicator">
